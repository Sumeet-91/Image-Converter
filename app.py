...
def make_grayscale(input_image):
	...
	pixelarray = np.fromstring(input_image, dtype='uint8')
	print('Image array', pixelarray)

	decodedarray = cv2.imdecode(pixelarray, cv2.IMREAD_UNCHANGED)
	print('Decoded Array',decodedarray)

	convertedimage = cv2.cvtColor(decodedarray,cv2.COLOR_RGB2GRAY)
	stats, outputimage = cv2.imencode('.png',convertedimage)
	print('Status',stats)
